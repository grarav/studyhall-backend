...
router.post("/:slug/seats/:seatNumber/assign", requireAdmin, async (req, res) => {
  const { name, mobile, aadhar, dob, gender, password, photoUrl, aadharPhotoUrl, paymentMethod } = req.body;
  ...
  try {
    ...
    const studentRes = await client.query(
      ...
      [hall.id, seat.id, name, dob || null, gender || null, mobile, aadhar || null, photoUrl || null, aadharPhotoUrl || null,
       passwordHash, feeAmount, join.toISOString().slice(0, 10), expiry.toISOString().slice(0, 10)]
    );
    await client.query(
