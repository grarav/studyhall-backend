...
router.get("/:slug/seats", requireAdmin, async (req, res) => {
  ...
  const result = await pool.query(
    `SELECT s.seat_number, s.is_locker, s.fee_amount, s.advance_amount,
            ...
            sa.id AS pending_application_id, sa.name AS pending_name
     FROM seats s
     LEFT JOIN students st ON st.seat_id = s.id
     LEFT JOIN signup_applications sa ON sa.seat_id_requested = s.id AND sa.status = 'pending'
     WHERE s.hall_id = $1
...
// Transactions for one specific date, e.g. ?date=2026-07-15
router.get("/:slug/transactions", requireAdmin, async (req, res) => {
  const hall = await getHallBySlug(req.admin.organizationId, req.params.slug);
  if (!hall) return res.status(404).json({ error: "Hall not found" });
  const { date } = req.query;
  if (!date || !/^\d{4}-\d{2}-\d{2}$/.test(date)) return res.status(400).json({ error: "Provide a date as ?date=YYYY-MM-DD" });

  const result = await pool.query(
    `SELECT p.id, p.amount, p.method, p.type, p.status, p.seat_number, p.created_at, st.name AS student_name
     FROM payments p
     LEFT JOIN students st ON st.id = p.student_id
     WHERE p.hall_id = $1 AND p.created_at::date = $2::date
     ORDER BY p.created_at DESC`,
    [hall.id, date]
  );
  const total = result.rows.filter((r) => r.status === "confirmed").reduce((sum, r) => sum + Number(r.amount), 0);
  res.json({ date, total, transactions: result.rows });
});
